...
export const buildCommand = async (options: any) => {
  ...
  try {
    ...
    const componentPaths = glob.sync(
      path.join(config.componentsDir, "**/component.json"),
    );

    if (componentPaths.length === 0) {
      spinner.warn(
        "No components found. Add components with: forge add <component-name>",
      );
      return;
    }
    ...
    for (const componentPath of componentPaths) {
      try {
        ...
        if (missingFiles.length > 0) {
          errors.push(
            `Component ${component.name} missing files: ${missingFiles.join(", ")}`,
          );
          continue;
        }

        // Additional validation
        const validationErrors = await validateComponent(
          component,
          componentDir,
        );
        if (validationErrors.length > 0) {
          errors.push(
            `Component ${component.name}: ${validationErrors.join(", ")}`,
          );
          continue;
        }
        ...
        if (options.verbose) {
          console.log(
            chalk.green(
              `✓ ${component.name} (${component.files.length} files)`,
            ),
          );
        }
      } catch (error) {
        errors.push(
          `Failed to process ${componentPath}: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }
    ...
    // Write registry file
    await fs.writeJSON(path.join(config.outputDir, "registry.json"), registry, {
      spaces: 2,
    });
    ...
    try {
      ...
      spinner.warn(
        "Storybook build failed. You may need to run npm install first.",
      );
    }
  ...
  <div class="max-w-4xl mx-auto px-4 py-12">
    ...
    <div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ${registry.categories
          .map((category: string) => {
            const count = registry.components.filter(
              (c: Component) => c.category === category,
            ).length;
            return `
...
<body class="bg-gray-50">
  ...
  <div class="max-w-4xl mx-auto px-4 py-8">
    ...
    <div class="mb-8">
      ...
      <ul class="space-y-1">
        ${component.dependencies
          ?.map(
            (dep: { name: string; version?: string; dev: boolean }) => `
    ...
    await fs.writeFile(
      path.join(docsDir, `${component.name}.html`),
      componentHtml,
    );
  }
}
...
async function generateIndexFiles(
  ...
  await fs.writeJSON(path.join(config.outputDir, "index.json"), index, {
    spaces: 2,
  });
}

async function generateDependencyManifest(
  config: ForgeConfig,
  components: Component[],
) {
  ...
  await fs.writeJSON(
    path.join(config.outputDir, "dependencies.json"),
    manifest,
    { spaces: 2 },
  );
}
