...
export async function listCommand(options: any) {
  try {
    ...
    if (componentPaths.length === 0) {
      console.log(
        chalk.yellow(
          "No components found. Add one with: forge add <component-name>",
        ),
      );
      return;
    }
    ...
    for (const [category, comps] of Object.entries(groupedByCategory)) {
      ...
      (comps as Component[]).forEach((comp: Component) => {
        const tags =
          comp.tags.length > 0 ? chalk.gray(`[${comp.tags.join(", ")}]`) : "";
        console.log(
          `  ${chalk.green(comp.name)} - ${comp.description} ${tags}`,
        );
      });
      console.log();
    }
...
export async function validateCommand(options: any) {
  ...
  try {
    const config = await loadForgeConfig();
    const { valid, invalid } = await validateAllComponents(
    ...
    if (invalid.length > 0) {
      ...
      invalid.forEach(
        ({ component, errors }: { component: string; errors: string[] }) => {
      ...
      if (options.fix) {
        ...
        console.log(
          chalk.gray(
            "Please fix the issues manually and run validation again.",
          ),
        );
      }
...
export async function publishCommand(options: any) {
  ...
  try {
    ...
    if (invalid.length > 0) {
      ...
      console.log(
        chalk.red("Fix validation errors first with: forge validate"),
      );
      return;
    }
    ...
    execSync(`gh-pages -d ${config.outputDir} -m "${message}"`, {
      stdio: "pipe",
    });
    ...
    try {
      ...
      if (packageJson.repository) {
        ...
        const githubUrl =
          repoUrl.replace(".git", "").replace("git+", "") + "/tree/gh-pages";
  ...
  try {
    ...
    results.forEach((comp) => {
      const tags =
        comp.tags.length > 0 ? chalk.gray(`[${comp.tags.join(", ")}]`) : "";
      console.log(
        `${chalk.green(comp.name)} (${comp.category}) - ${comp.description} ${tags}`,
      );
    });
  } catch (error) {
    console.error(
...
export async function updateCommand(componentName: string) {
  ...
  console.log(
    chalk.blue(
      "You can manually edit the component files and run: forge build",
    ),
  );
}
  ...
  try {
    ...
    if (options.get) {
      ...
      if (value !== undefined) {
        ...
        console.error(
          chalk.red(`Configuration key "${options.get}" not found`),
        );
      }
      return;
    }
