  ...
  try {
    ...
    if (results.length === 0) {
      ...
      if (options.category) {
        console.log(
          chalk.gray(
            `Tip: Try searching without --category=${options.category}`,
          ),
        );
      }
      return;
    ...
    console.log(
      chalk.blue(`Found ${results.length} components matching "${query}":`),
    );
    console.log();
    ...
    console.log(
      chalk.gray(
        "Tip: Use `forge add <component-name>` to add a component to your project",
      ),
    );
  } catch (error) {
