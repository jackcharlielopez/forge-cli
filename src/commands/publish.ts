...
export async function publishCommand(options: PublishOptions = {}) {
  ...
  try {
    ...
    if (validateResult.exitCode !== 0) {
      ...
      console.error(
        chalk.red("Error: All components must be valid before publishing"),
      );
      return;
    }

    // Get current Git branch
    const { stdout: branchOutput } = await run_in_terminal(
    ...
    const { stdout: statusOutput } = await run_in_terminal(
      "git status --porcelain",
    );
    if (statusOutput.trim() !== "") {
      spinner.warn("You have uncommitted changes");
      console.log(
        chalk.yellow("Please commit or stash your changes before publishing"),
      );
      return;
    }
    ...
    console.log(
      chalk.gray(
        "  1. GitHub Actions will build and deploy your component library",
      ),
    );
    console.log(
      chalk.gray("  2. Check the Actions tab on GitHub for build status"),
    );
    console.log(
      chalk.gray(
        "  3. Once deployed, your components will be available on GitHub Pages",
      ),
    );
  } catch (error) {
