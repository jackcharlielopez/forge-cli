...
export async function addCommand(componentName?: string, options?: any) {
  ...
  try {
    ...
    const template =
      (component as any).template || options?.template || "basic";
    const useTypeScript = (component as any).typescript !== false;
    // Generate files based on template
    const templateData = await generateTemplate(
      template,
      componentName!,
      useTypeScript,
    );
    ...
    console.log(
      chalk.yellow("  2. Test your component: ") +
        chalk.white(`cd ${componentDir}`),
    );
    console.log(
      chalk.yellow("  3. Build library: ") + chalk.white("forge build"),
    );
  } catch (error) {
